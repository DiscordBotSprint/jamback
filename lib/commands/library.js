...
async function subcommandSearch(interaction) {
  ...
  const returnedSongs = await Song.getByTitle(query, guildId);
  if (!returnedSongs) {
  ...
  // is there any reason you're not passing these directly into the class constructor?
  // i'm assuming its for clarity of code?
...
async function subcommandView(interaction) {
  const returnedSongs = await Song.getAll(interaction.guild.id);
  const pageSize = 8;
